 ...
 * Use the {@link GameFactory} class the get instances of this class.
 */
public class Game {
   ...
   * with 3 numbers at the most, being the first the move ('0' to draw a card,
   * '1' to play from the hand), the second the chosen colour in case of an wild
   * card ('0' for blue, '1' for red, '2' from green and '3' for yellow) and the
   * last being the index of the card the player wishes to play in case he chose
   * to do so.
   *
  ...
  private void checkCommandValidity(GameCommand command)
    ...
    if (command.getOption() == 0) {
      if (command.getIndex() != -1 || command.getColour() != null) {
        throw new IllegalStateException("Not expecting values for index and colour" +
            "when option is 0");
      }
    }
      ...
      if (command.getIndex() > players[turn].getHand().length() - 1)
        throw new CardIndexOutOfHandBoundsException(String.format(
            "%d is out of hand range of %d", command.getIndex(),
            players[turn].getHand().length()
        ));
    ...
    else throw new InvalidOptionException("Invalid option");
  }
  ...
  private void doCaseSpecial() {
    ...
    if (tableTop.getSymbol() == SpecialCardSymbols.DRAW_2) {
      ...
      revert();
      if (nOfPlayers > 2) moveToNextPlayer();
    } else { // skip card
      jumpAPlayer();
    }
  }
